    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        ...
        <section aria-label={t('content.title')} className="content">
          ...
          {!activeProject && activeLibrary === null && !showProjects && <section aria-label={t('projects.filter')} className="library-filters"><div><span>{t('projects.filter')}</span><div className="filter-chips">{projects.map((project) => <button aria-label={`${t('projects.filter')}: ${project.name}`} aria-pressed={libraryProjectFilters.includes(project.id)} className={`filter-chip ${libraryProjectFilters.includes(project.id) ? 'is-active' : ''}`} key={project.id} onClick={() => setLibraryProjectFilters((current) => current.includes(project.id) ? current.filter((id) => id !== project.id) : [...current, project.id])} type="button">{project.name}</button>)}</div></div><div><span>{t('tags.filter')}</span><div className="filter-chips">{tags.map((tag) => <button aria-label={`${t('tags.filter')}: ${tag.name}`} aria-pressed={libraryTagFilters.includes(tag.key)} className={`filter-chip ${libraryTagFilters.includes(tag.key) ? 'is-active' : ''}`} key={tag.key} onClick={() => setLibraryTagFilters((current) => current.includes(tag.key) ? current.filter((key) => key !== tag.key) : [...current, tag.key])} type="button">{tag.name}</button>)}</div></div></section>}
